...
const FormValidation = () => {
  ...
  const [obj, setobj] = useState({ hobbies: [], fname: '', lname: '', email: '', phone: '', date: '', password: '', confirmPassword: '', gender: '', information: '' });
  const [array, setarray] = useState([]);
  let [isError, setisError] = useState({});
  const [formHeight, setformHeight] = useState(false)
  let [filterObject, setfilterObject] = useState({fname : '', lname : '', email : ''})
  ...
  const save = () => {
    ...
    /* AHI APRNE BDHI KEY MLSE */
    for (let key in obj) {

      validateData(key);
    }

    // console.log(Object.keys(isError), isError)
  ...
  const validateData = (name) => {
    if (name == 'fname') {
      if (!obj.fname || obj.fname == "") {
        ...
        delete isError.fname;  /* DELETE KARVATHI JEMA ERROR NAI HSE A KEY DELETE THAI JSE SAME BDHA MA AA KARVANU */
      }
    }
  ...
  const getFilterData = (e)=>{
      ...
      filterArray = array.filter( x => x.fname.toLowerCase().includes(filterObject.fname.toLowerCase()) &&  x.lname.toLowerCase().includes(filterObject.lname.toLowerCase()) && x.email.toLowerCase().includes(filterObject.email.toLowerCase()))
      setfilterArray([...filterArray])
  ...
  return (
    <Fragment>
      ...
      <form action="" className="w-50 mx-auto border border-1 p-4 mt-5 form-custom" style={{height:formHeight ? 'auto' : '70px'}}>
